    ...
    const calculateTotalPostOfEveryUser = (posts) => {
        ...
        for (const post of posts) {
            ...
            if (userId in postCountByUser) {
                ...
                // If the user doesn't exist in the object, initialize their post count to 1
    ...
    return (
        <>
            ...
            {users.map((user, index) => (
                <div key={`user-${index}`} className=' border-[1.5px] border-solid border-black rounded-lg h-16 mb-2 bg-blue-200 shadow-[1px_1px_10px_0px_rgba(0,0,0,0.5)] mx-8 hover:bg-blue-400 hover:cursor-pointer grid grid-cols-2 ' onClick={() => getUserAllDetails(user.id)}>
